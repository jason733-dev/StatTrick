# StaTrick — Bangers Match Tracker (Vercel + Gemini + Firebase)
...
`api/storage.js` (Firebase Firestore) and `api/summary.js` (Google Gemini)
— so it runs on your own domain, no Claude account or plan required for
viewers.
...
- **`api/summary.js`** — generates the AI game recap by calling **Google
  Gemini** with your key, kept server-side only — the browser never sees
  it.
...
Go to [aistudio.google.com/apikey](https://aistudio.google.com/apikey), sign in, click **Create API key**. Free tier is generous for a team app like this.
...
- The Gemini model used is `gemini-2.5-flash`. If Google retires that
  string later, check [ai.google.dev/gemini-api/docs/models](https://ai.google.dev/gemini-api/docs/models)
  for the current one and swap it into `api/summary.js`.
